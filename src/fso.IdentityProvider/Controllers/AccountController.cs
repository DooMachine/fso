...
using fso.IdentityProvider.Controllers;
using fso.IdentityData.Domains;
using fso.IdentityProvider.Services;
using fso.IdentityProvider.Models.AccountViewModels;
...
namespace IdentityServerWithAspNetIdentity.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Logout(LogoutViewModel model)
        {
            ...
            await _persistedGrantService.RemoveAllGrantsAsync(subjectId, "fso.AngularClient");
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailSender.SendEmailAsync(model.Email, "Feasion Accounts - Confirm",
                        $"Please confirm your account by clicking this link: <a href='{callbackUrl}'>link</a>");
                    await _signInManager.SignInAsync(user, isPersistent: true);

                    var role = await _roleManager.FindByNameAsync("fso.api.user");
                    if (role==null)
                    {
                        await _roleManager.CreateAsync(new IdentityRole() { Name = "fso.api.user" });
                    }
                    await _userManager.AddToRoleAsync(user, "fso.api.user");
                    await _eventBus.PublishAsync(new UserRegisteredAction()
                    {                        
                        UserId = user.Id,
                        Name = user.Name,
